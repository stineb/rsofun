...
The metabolic leaf N ($N_v$) is taken to be proportional to $V_\mathrm{cmax}$ at standard temperature (25°C, $V_\mathrm{cmax25}$).
$$
\begin{align}
N_v &= b_v V_{cmax25} \\
    &= \underbrace{c_0 b_v f_t(T)^{-1}}_{c_0^\ast}\left(1 - e^{-kL} \right)
\end{align}
$$
...
C_\mathrm{fol} &= C^l L \\
               ...
               &= L b_c (a_s + b_s N_v^l) \\
               &= L b_c a_s + b_s N_v \\ 
               &= L b_c a_s + b_s c_0^\ast \left(1 - e^{-kL} \right)
\end{aligned}
$$
...
- $N_v, N_\mathrm{fol}, C_\mathrm{fol}, (C:N)_\mathrm{fol}$, and LMA versus LAI.
- LAI versus $C_\mathrm{fol}$

Define and obtain values representing a temperate forest with a LAI of 5.
...
### State variables
The value for $c_0$ is simulated dynamically in response to the environment. It is
```{r}
...
Load library for Lambert-W function
```{r}
library(LambertW)
```
