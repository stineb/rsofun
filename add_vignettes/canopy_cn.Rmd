...
$V_\mathrm{cmax}$ at standard temperature (25°C, $V_\mathrm{cmax25}$).

$$
\begin{aligned}
N_v &= b_v V_{cmax25} \\
    &= b_v f_t^{-1} c_0 \left(1 - e^{-kL} \right)
\end{aligned}
$$
...
N_\mathrm{fol} &= L (N_v^l + N_s^l) \\
               &= a_s L + (1 + b_s)N_v \\
               &= a_s L + (1 + b_s) c_0 b_v f_t^{-1} \left(1 - e^{-kL} \right)
\end{aligned}
$$
...
C_\mathrm{fol} &= C^l L \\
               ...
               &= L b_c a_s + b_c b_s c_0 b_v f_t^{-1} \left(1 - e^{-kL} \right)
\end{aligned}
$$
...
-   $N_v$, $N_\mathrm{fol}$, $C_\mathrm{fol}$, $(C:N)_\mathrm{fol}$, and
    LMA versus LAI.
-   $(C:N)_\mathrm{fol}$ versus $C_\mathrm{fol}$
...
df |> 
  pivot_longer(cols = c(cn_fol, lma, c_fol, n_fol, n_v),
               names_to = "var",
               values_to = "val") |> 
  ggplot(aes(lai, val)) +
  geom_line() +
